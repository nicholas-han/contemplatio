...
function handlerFor(name: string, tools: EquityResearchTools): (args: any, exec: unknown) => unknown | Promise<unknown> {
  const handlers: Record<string, (args: any, exec: unknown) => unknown | Promise<unknown>> = {
    ...
    addIndustry: (args) => tools.addIndustry(args.companyId, args.input),
    addBusinessLine: (args) => tools.addBusinessLine(args.companyId, args.companyIndustryId, args.input),
    getTaxonomy: (args) => tools.getTaxonomy(args.companyId),
    getEstimates: (args) => tools.getEstimates(args.companyId, args.metricId, args.limit, args.asOfFrom, args.asOfTo, args.targetPeriodEnd, args.companyIndustryId, args.businessLineId),
