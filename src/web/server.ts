  ...
  private handle(request: IncomingMessage, response: ServerResponse): void {
    try {
      ...
      if (url.pathname === '/' || url.pathname.startsWith('/companies/')) {
        ...
        return sendHtml(response, renderPage(pathCompanyId, this.dataEngine.listLegacyObservations(pathCompanyId), this.dataEngine.listFacts(pathCompanyId), this.dataEngine.listPeople(pathCompanyId), this.dataEngine.listCapTable(pathCompanyId), this.dataEngine.listSources(pathCompanyId), this.dataEngine.listEstimates(pathCompanyId), this.modelEngine.listScenarios(pathCompanyId), this.modelEngine.listRuns(pathCompanyId)))
      }
...
function renderPage(companyId: string, observations: LegacyObservationRecord[], facts: ReturnType<EquityDataEngine['listFacts']>, people: ReturnType<EquityDataEngine['listPeople']>, capTable: ReturnType<EquityDataEngine['listCapTable']>, sources: ReturnType<EquityDataEngine['listSources']>, estimates: ReturnType<EquityDataEngine['listEstimates']>, scenarios: ReturnType<EquityModelEngine['listScenarios']>, runs: ReturnType<EquityModelEngine['listRuns']>): string {
  ...
  return `<!doctype html><html lang="en"><head><meta charset="utf-8"><meta name="viewport" content="width=device-width,initial-scale=1">
    ...
    <h2>Import Management CSV</h2><form method="post" action="/api/management/import"><input type="hidden" name="company_id" value="${escapeHtml(companyId)}"><textarea name="csv" rows="6" style="width:100%;font-family:monospace" placeholder="name_zh,name_en,birth_year,unit_name,unit_type,role_title_raw,role_type,position_name_normalized,start_date,end_date,is_current,source_id,evidence_id"></textarea><p><button name="apply" value="false" type="submit">Preview</button> <button name="apply" value="true" type="submit">Import</button></p></form>
