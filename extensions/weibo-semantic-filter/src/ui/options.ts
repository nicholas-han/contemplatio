...
el('settings-form').addEventListener('submit',event=>{
  event.preventDefault();let next:Settings;try{next=collect();}catch(e){status((e as Error).message,true);return;}
  // permissions.request must begin in the user gesture, before any asynchronous work.
  ...
  void permission.then(async granted=>{if(!granted)throw Error('模型域名未授权，设置尚未保存');const result=await rpc({type:'saveSettings',settings:next,apiKey:el<HTMLInputElement>('apiKey').value,clearKey:el<HTMLInputElement>('clearKey').checked});settings=result.settings;el<HTMLInputElement>('apiKey').value='';el<HTMLInputElement>('apiKey').placeholder=result.hasKey?'已保存；留空保持当前 key':'填写后保存在扩展本地';el<HTMLInputElement>('clearKey').checked=false;fill();status('已保存');}).catch(e=>status(e.message,true));
});
el('test').addEventListener('click',()=>{
  const button=el<HTMLButtonElement>('test');button.disabled=true;
  ...
  report('正在测试…');void rpc({type:'testConnection'}).then(r=>report(`连接成功 · ${r.model} · ${r.latencyMs} ms`)).catch(e=>report(e.message,true)).finally(()=>{button.disabled=false;});
});
...
void rpc({type:'getSettings'}).then(data=>{settings=data.settings;fill();el<HTMLInputElement>('apiKey').placeholder=data.hasKey?'已保存；留空保持当前 key':'填写后保存在扩展本地';el('stats').textContent=`本次浏览器运行：模型调用 ${data.stats.calls} 次 · 缓存命中 ${data.stats.cacheHits} 次 · 错误 ${data.stats.errors} 次。实际费用请以服务商账单为准。`;}).catch(e=>status(e.message,true));
